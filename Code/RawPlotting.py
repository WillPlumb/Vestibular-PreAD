...
path='C:/Users/Will/DropBox/Disso/Data'
os.chdir(path)
...
def movementChange(Trial, participant):
    if Trial==5 and (participant in [7,19,22,54,58,60,61,66,78,87,88,93,95,120,125,142,143,144]):
        trialNew = [90,-120,120,300,15,100,65,95,-80,90,-120,225,340,-235,-300,325,100,-65,152]
        return trialNew
    elif (Trial in [7,8,13,14,15,16,17,18]) and (participant in[2,3,4,6,8,10,12,17,20,27,30,1,32,35,37,38,42,43,56,59,73]):
        trialNew = [90,-120,120,300,-85,100,125,-95,-80,90,-120,225,225,300,125,100,65,150,152]
        return trialNew
    else:
        trial = [90,-120,120,300,-85,100,65,95,-80,90,-120,225,340,-235,-300,325,100,-65,152]
        return trial
...
def OrientationError(data,Trial,participant):
    trialNew = movementChange(Trial,participant)
    ...
    if trialNew[Trial-1]<0 and (data.loc[1,' Magnetic'] - data.loc[1,' Reference Orientation'] >0):
        data[' Orientation Error']=data[' Orientation Error']-360
    elif trialNew[Trial-1]>0 and (data.loc[1,' Magnetic'] - data.loc[1,' Reference Orientation'] <0):
        data[' Orientation Error']=data[' Orientation Error']+360
    ...
    if data.loc[len(data.index)-1,' Orientation Error']<trialNew[Trial-1] and trialNew[Trial-1]<0:
        data[' Orientation Error']=-(data[' Orientation Error']-data.loc[1,' Orientation Error'])+data.loc[1,' Orientation Error']
    elif data.loc[len(data.index)-1,' Orientation Error']>trialNew[Trial-1] and trialNew[Trial-1]>0:
        data[' Orientation Error']=-(data[' Orientation Error']-data.loc[1,' Orientation Error'])+data.loc[1,' Orientation Error']    
...
def trialChange(trial, participant):
    trial = int(trial)
    if trial==5 and (participant in [7,19,22,54,58,60,61,66,78,87,88,93,95,120,125,142,143,144]):
        trialNew = str(trial) + 'a'
        return trialNew
    elif (trial in [7,8,13,14,15,16,17,18]) and (participant in[2,3,4,6,8,10,12,17,20,27,30,1,32,35,37,38,42,43,56,59,73,79,80,129]):
        trialNew = str(trial) + 'a'
        return trialNew
    else:
        return trial
...
def Trial_gyro_summary(Trial):
    count1 = 0
    count2 = 0
    if Trial[-1]=='a':
        TrialOld = Trial[:-1]
        Trial_number = "*trial_"+Trial[:-1]+"_*"
        output2 = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + Trial[:-1] + "\\" + "Trial" + Trial[:-1]+"GyroSummary.csv"
        if os.path.isfile(output2):    
            os.remove(output2)
    else:
        TrialOld= Trial
        Trial_number = "*trial_"+Trial+"_*"
        output2 = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + Trial + "\\" + "Trial" + Trial+"GyroSummary.csv"
        if os.path.isfile(output2):    
            os.remove(output2)
    directory = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + Trial
    output1 = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + Trial + "\\" + "Trial" + Trial+"GyroSummary.csv"
    ...
    for filename in os.listdir(os.getcwd()):
        for file in os.listdir(filename):
             if fnmatch.fnmatch(file, 'gyroscope_data*') and fnmatch.fnmatch(file, Trial_number):
                ...
                if str(data.iloc[0][' Trial'])[-1]=='a' and count2==0:
                    data.to_csv(output1, mode='a', header=True)
                    count2=count2+1
                elif str(data.iloc[0][' Trial'])[-1]=='a' and count2!=0:
                    data.to_csv(output1, mode='a', header=False)
                elif count1==0:
                    data.to_csv(output2, mode='a', header=True)
                    count1=count1+1
                else:
                    data.to_csv(output2, mode='a', header=False)
    ...
    for k in range(0,count1+count2):
        if k == 0:
            ...
            temp = Trial[:-1]
        output = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + temp + "\\" + "Trial" + temp+"GyroSummary.csv"
        data = pandas.read_csv(output, index_col='Participant')
        
        fig, ax = plt.subplots() 
        for key, grp in data.groupby(['Participant']):
            ax = grp.plot(ax=ax, kind='line', x=' Milliseconds Since Trial Start', y=' X Rotation (Deg)',legend=False)
        plt.ylabel('X Rotation (Deg)')
        xoutput = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + temp + "\\" + "Trial" + temp+"GyroXRotations.png"
        plt.savefig(xoutput)
        plt.show()
        
        fig, ax = plt.subplots() 
        for key, grp in data.groupby(['Participant']):
            ax = grp.plot(ax=ax, kind='line', x=' Milliseconds Since Trial Start', y=' Y Rotation (Deg)',legend=False)
        plt.ylabel('Y Rotation (Deg)')
        youtput = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + temp + "\\" + "Trial" + temp+"GyroYRotations.png"
        plt.savefig(youtput)
        plt.show()
        
        fig, ax = plt.subplots() 
        for key, grp in data.groupby(['Participant']):
            ax = grp.plot(ax=ax, kind='line', x=' Milliseconds Since Trial Start', y=' Z Rotation (Deg)',legend=False)
        plt.ylabel('Z Rotation (Deg)')
        youtput = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + temp + "\\" + "Trial" + temp+"GyroZRotations.png"
        plt.savefig(youtput)
        plt.show()
 
    
    
def Trial_accel_summary(Trial):
    count1 = 0
    count2 = 0
    output = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + Trial + "\\" + "Trial" + Trial+"AccelSummary.csv"
    if Trial[-1]=='a':
        TrialOld = Trial[:-1]
        Trial_number = "*trial_"+Trial[:-1]+"_*"
        output2 = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + Trial[:-1] + "\\" + "Trial" + Trial[:-1]+"AccelSummary.csv"
        if os.path.isfile(output2):    
            os.remove(output2)
    else:
        TrialOld = Trial
        Trial_number = "*trial_"+Trial+"_*"
        output2 = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + Trial + "\\" + "Trial" + Trial+"AccelSummary.csv"
        if os.path.isfile(output2):    
            os.remove(output2)
    directory = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + Trial
    output1 = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + Trial + "\\" + "Trial" + Trial+"AccelSummary.csv"
    if not os.path.exists(directory):
        os.makedirs(directory)
    if os.path.isfile(output1):    
        os.remove(output1)       
    for filename in os.listdir(os.getcwd()):
        for file in os.listdir(filename):
             if fnmatch.fnmatch(file, 'accelerometer_data*') and fnmatch.fnmatch(file, Trial_number):
                datafile = os.getcwd() + "\\" + filename + "\\" + file
                data = pandas.read_csv(datafile)
                geneTypeFile = os.getcwd() + "\\Personal\\Cohort - demographics.csv"
                geneType = pandas.read_csv(geneTypeFile, index_col='ID')
    ...
    for k in range(0,count1+count2):
        if k == 0:
            ...
            temp = Trial[:-1]
        output = os.getcwd()[:-4] + "Python" + "\\" + "Trial" + temp + "\\" + "Trial" + temp+"AccelSummary.csv"
        data = pandas.read_csv(output, index_col='Participant')
    
        fig, ax = plt.subplots() 
        for key, grp in data.groupby(['Participant']):
            ax = grp.plot(ax=ax, kind='line', x=' Milliseconds Since Trial Start', y=' X Gravity',legend=False)
        plt.ylabel('X Gravity')
        xoutput = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + temp + "\\" + "Trial" + temp+"AccelXRotations.png"
        plt.savefig(xoutput)
        plt.show()
        
        fig, ax = plt.subplots() 
        for key, grp in data.groupby(['Participant']):
            ax = grp.plot(ax=ax, kind='line', x=' Milliseconds Since Trial Start', y=' Y Gravity',legend=False)
        plt.ylabel('Y Gravity')
        youtput = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + temp + "\\" + "Trial" + temp+"AccelYRotations.png"
        plt.savefig(youtput)
        plt.show()
        
        fig, ax = plt.subplots() 
        for key, grp in data.groupby(['Participant']):
            ax = grp.plot(ax=ax, kind='line', x=' Milliseconds Since Trial Start', y=' Z Gravity',legend=False)
        plt.ylabel('Z Gravity')
        youtput = os.getcwd()[:-4] + "\\" + "Python" + "\\" + "Trial" + temp + "\\" + "Trial" + temp+"AccelZRotations.png"
        plt.savefig(youtput)
        plt.show()
...
def Trial_heading_summary(Trial):
    ...
    for k in range(0,count1+count2):
        ...
        plt.title('Heading Orientation Error for Trial ' + temp)
    #    axes = plt.gca()
