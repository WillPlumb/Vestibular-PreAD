...
def correlationFS(trials,varience):
    ...
    for k in trials:
        ...
        """CHANGE TO REMOVE FEATURES"""
        #df = df.drop(['Age','Gender', 'Occupation'],axis=1)
        df = df.drop([' End Error',' Normalised End Error'],axis=1)
...
def main():
    ...
    feats = []
    trials = [1,2,3,4,5,6,'7a','8a',9]
    corrSet = [0.05,0.1,0.2,0.3,0.5]
    for corr in corrSet:
        print("corr",corr)
        corfeatures = correlationFS([1,2,3,4,5,6,'7a','8a',9],corr)
        feats.append(corfeatures)
    output =[]
    for corSet in range(0,len(feats)):
        for trial in range(0,len(trials)):
            output.append([trials[trial],corrSet[corSet],feats[corSet][trial]])
    
    output = pandas.DataFrame(data=output,columns=['Trial','Correlation Threshold','Features'])
    
    output.to_csv('C:/Users/willp/DropBox/Disso/Python/Models/CorrTemp.csv',index=False)
    ...
    printout.to_csv('C:/Users/willp/DropBox/Disso/Python/Models/RFF1_NOENDERROR.csv',index=False)
    ...
    for corr in [1.0]:
        ...
        RF['Correlation threshold'] = corr
        printout = printout.append(RF) 
    printout.to_csv('C:/Users/willp/DropBox/Disso/Python/Models/RFSummary_NOENDERROR.csv',index=False)
    ...
    SVMout.to_csv('C:/Users/willp/DropBox/Disso/Python/Models/SVMSummary_NOENDERROR.csv',index=False)
    ...
    MLPout.to_csv('C:/Users/willp/DropBox/Disso/Python/Models/MLPSummary_NOENDERROR.csv',index=False)
...
#    printout = printout.append(RF)
#    printout.to_csv('C:/Users/willp/Dropbox/Disso/Python/Models/ALLRF.csv',index=False)
